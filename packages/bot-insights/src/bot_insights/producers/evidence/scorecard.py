...
``soc_triage``, ``crawler_governance``, ``edge_ops_impact``. The
per-report interpretation_contract + template-sections are kept as
module-level constants so the report-type branches in
...
from producers.evidence.scorecard_fleet import build_scorecard_fleet_evidence_packet
