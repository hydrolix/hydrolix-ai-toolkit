...
DATASET_SCHEMA = "threat_hunt_dataset.v1"
PROVENANCE_OBSERVED = "observed"
PROVENANCE_SYNTHETIC_UNAVAILABLE = "synthetic_unavailable"
PROVENANCE_ESTIMATED = "estimated"
PROVENANCE_DERIVED_FROM_OBSERVED = "derived_from_observed"
VALID_FIELD_PROVENANCE = {
    PROVENANCE_OBSERVED,
    PROVENANCE_SYNTHETIC_UNAVAILABLE,
    PROVENANCE_ESTIMATED,
    PROVENANCE_DERIVED_FROM_OBSERVED,
}
...
_PROVENANCE_RECORDER = None
_PROVENANCE_STAGE_STACK: list[str] = []
        ...
        if value.get("schema_version") == DATASET_SCHEMA:
            ...
            if isinstance(rows, list):
                ...
                if isinstance(field_provenance, dict):
                    for row in out:
                        row.setdefault("__field_provenance", field_provenance)
                if dataset_metadata:
                    for row in out:
                        row.setdefault("__dataset_metadata", dataset_metadata)
                return out
...
def _columns_for_rows(rows: Iterable[dict[str, Any]]) -> list[str]:
    columns: list[str] = []
    seen: set[str] = set()
    for row in rows:
        for key in row:
...
def _field_provenance(
    ...
    *,
    default: str = PROVENANCE_OBSERVED,
    overrides: dict[str, str] | None = None,
) -> dict[str, str]:
    provenance = {column: default for column in columns}
    provenance.update(overrides or {})
    invalid = {
        field: state
        for field, state in provenance.items()
        if state not in VALID_FIELD_PROVENANCE
    }
    if invalid:
        detail = ", ".join(f"{field}={state}" for field, state in sorted(invalid.items()))
        raise SystemExit(f"Invalid field provenance state(s): {detail}")
    return provenance


def _dataset_sidecar_paths(path: Path) -> list[Path]:
    return [
        path.with_suffix(path.suffix + ".manifest.json"),
        path.with_suffix(path.suffix + ".provenance.json"),
        path.parent / f"{path.name}.manifest.json",
        path.parent / f"{path.name}.provenance.json",
    ]


def _read_dataset_manifest(path: Path) -> dict[str, Any] | None:
    if path.suffix in {".json", ".jsonl"}:
        try:
            value = json.loads(path.read_text(encoding="utf-8") or "null")
        except (OSError, ValueError, TypeError):
            value = None
        if isinstance(value, dict) and value.get("schema_version") == DATASET_SCHEMA:
            return value
    for sidecar in _dataset_sidecar_paths(path):
        if not sidecar.exists():
            continue
        try:
            value = json.loads(sidecar.read_text(encoding="utf-8") or "null")
        except (OSError, ValueError, TypeError):
            continue
        if isinstance(value, dict) and value.get("schema_version") == DATASET_SCHEMA:
            return value
...
def validate_replay_grade_dataset(path: Path, *, role: str) -> dict[str, Any]:
    manifest = _read_dataset_manifest(path)
    if not manifest:
        raise SystemExit(
            f"full-required {role} input lacks {DATASET_SCHEMA} wrapper or sidecar manifest: {path}"
        )
    row_count = manifest.get("row_count")
    total_row_count = manifest.get("total_row_count")
    if role == "summary":
        _validate_selected_summary_manifest(manifest, path)
    elif manifest.get("truncated") is not False:
        raise SystemExit(f"full-required {role} input is truncated or missing truncated=false: {path}")
    if not isinstance(row_count, int) or not isinstance(total_row_count, int):
        raise SystemExit(f"full-required {role} input is missing row-count metadata: {path}")
    if role != "summary" and row_count < total_row_count:
        raise SystemExit(
            f"full-required {role} input is lossy: row_count={row_count}, total_row_count={total_row_count}"
        )
    field_provenance = manifest.get("field_provenance")
    if not isinstance(field_provenance, dict):
        raise SystemExit(f"full-required {role} input is missing field provenance: {path}")
    return manifest


def _validate_selected_summary_manifest(manifest: dict[str, Any], path: Path) -> None:
    row_count = manifest.get("row_count")
    total_row_count = manifest.get("total_row_count")
    metadata = manifest.get("metadata")
    selection = metadata.get("selection") if isinstance(metadata, dict) else None
    if not isinstance(row_count, int) or not isinstance(total_row_count, int):
        raise SystemExit(f"full-required summary input is missing row-count metadata: {path}")
    if not isinstance(metadata, dict) or not isinstance(selection, dict):
        raise SystemExit(f"full-required summary input is missing selected Top-N metadata: {path}")
    if metadata.get("population_complete") is not False or selection.get("population_complete") is not False:
        raise SystemExit(f"full-required summary input must declare population_complete=false: {path}")
    if metadata.get("evidence_role") != "selected_summary_evidence" or selection.get("evidence_role") != "selected_summary_evidence":
        raise SystemExit(f"full-required summary input must declare selected_summary_evidence role: {path}")
    if selection.get("mode") != "top_n_by_period":
        raise SystemExit(f"full-required summary input has unsupported selection mode: {path}")
    if selection.get("limit_per_period") != SUMMARY_EXPORT_ROWS_PER_PERIOD:
        raise SystemExit(f"full-required summary input has unsupported selection limit: {path}")
    expected_order = [
        "period",
        "requests DESC",
        "request_path ASC",
    ...
    if selection.get("order_by") != expected_order:
        raise SystemExit(f"full-required summary input has malformed selection order: {path}")
    candidate_rows_by_period = selection.get("candidate_rows_by_period")
    if not isinstance(candidate_rows_by_period, dict):
        raise SystemExit(f"full-required summary input is missing candidate_rows_by_period: {path}")
    for period in ("current", "baseline"):
        if not isinstance(candidate_rows_by_period.get(period), int):
            raise SystemExit(f"full-required summary input is missing {period} candidate count: {path}")
    candidate_row_count = selection.get("candidate_row_count")
    selected_row_count = selection.get("selected_row_count")
    if candidate_row_count != total_row_count:
        raise SystemExit(f"full-required summary candidate_row_count must equal total_row_count: {path}")
    if selected_row_count != row_count:
        raise SystemExit(f"full-required summary selected_row_count must equal row_count: {path}")
    if sum(candidate_rows_by_period.values()) != candidate_row_count:
        raise SystemExit(f"full-required summary candidate counts do not sum to total_row_count: {path}")
    if row_count > total_row_count:
        raise SystemExit(f"full-required summary row_count exceeds total_row_count: {path}")
    if manifest.get("truncated") is not (row_count < total_row_count):
        raise SystemExit(f"full-required summary truncated flag does not match selected/candidate counts: {path}")
    if not selection.get("selection_sql_sha256") or not selection.get("candidate_count_sql_sha256"):
        raise SystemExit(f"full-required summary input is missing selection SQL hashes: {path}")


def validate_replay_grade_glob(pattern: str, *, role: str) -> list[dict[str, Any]]:
    paths = [Path(p) for p in sorted(glob.glob(pattern))]
    if not paths:
        raise SystemExit(f"--summary-parquet-glob matched no files: {pattern}")
    return [validate_replay_grade_dataset(path, role=role) for path in paths]


def _attach_dataset_manifest(rows: list[dict[str, Any]], manifest: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not manifest:
        return rows
    field_provenance = manifest.get("field_provenance")
    dataset_metadata = {
...
def write_threat_hunt_dataset(
    ...
    *,
    ...
    payload = {
        ...
        "query_sql": query_sql,
        "query_sha256": hashlib.sha256((query_sql or "").encode("utf-8")).hexdigest()
        if query_sql is not None
        else None,
        "columns": columns,
        "rows": rows,
        "row_count": len(rows),
        "total_row_count": len(rows) if total_row_count is None else int(total_row_count),
        "truncated": bool(truncated),
        "chunk_index": int(chunk_index),
        "chunk_count": int(chunk_count),
        "started_at": started_at,
        "finished_at": finished_at,
        "field_provenance": field_provenance or _field_provenance(columns),
        "output_sha256": _sha256_json({"columns": columns, "rows": rows}),
    }
    if metadata:
    ...
    *,
    ...
    if replay_grade:
        ...
        write_threat_hunt_dataset(
            ...
            or _field_provenance(columns, default=PROVENANCE_DERIVED_FROM_OBSERVED),
...
def set_provenance_recorder(recorder) -> None:
    global _PROVENANCE_RECORDER
    _PROVENANCE_RECORDER = recorder


@contextmanager
def provenance_stage(stage_id: str):
    _PROVENANCE_STAGE_STACK.append(stage_id)
    try:
        yield
    finally:
        _PROVENANCE_STAGE_STACK.pop()


def _record_provenance_event(event: dict[str, Any]) -> None:
    if _PROVENANCE_RECORDER is None:
        return
    _PROVENANCE_RECORDER(event)
...
def read_rows_from_glob(pattern: str) -> list[dict[str, Any]]:
    ...
    for path in paths:
        ...
        if path.suffix == ".parquet":
            file_rows = _read_parquet_rows(path)
        elif path.suffix in {".json", ".jsonl"}:
            file_rows = _read_json_rows(path)
        ...
        rows.extend(_attach_dataset_manifest(file_rows, _read_dataset_manifest(path)))
    return rows


def _provenance_state(row: dict[str, Any], field: str) -> str:
    provenance = row.get("__field_provenance")
    if not isinstance(provenance, dict):
        return PROVENANCE_OBSERVED
    state = provenance.get(field)
    return str(state) if state else PROVENANCE_OBSERVED


def _provenance_value(row: dict[str, Any], field: str, aliases: tuple[str, ...], default: Any = None) -> Any:
    if _provenance_state(row, field) == PROVENANCE_SYNTHETIC_UNAVAILABLE:
        return default
    return _first(row, aliases, default)


def _normalize_summary_row(row: dict[str, Any]) -> dict[str, Any]:
    requests = _num(_first(row, ("requests", "request_count", "count", "hits")))
    response_body_bytes_raw = _provenance_value(
        row,
        "response_body_bytes",
        ("response_body_bytes", "response_bytes", "bytes"),
    )
    response_body_bytes = _num(response_body_bytes_raw)
    ...
    status_429 = _num(
        _provenance_value(
            row,
            "status_429",
            ("status_429", "requests_429", "429", "rate_limited_requests", "req_429"),
        )
    )
    ...
    return {
        ...
        "traffic_cohort": str(
            _provenance_value(
                row,
                "traffic_cohort",
                ("trafficCohort", "traffic_cohort", "cohort", "bot_class"),
                "",
            )
        ),
        "bot_requests": _num(
            _provenance_value(
                row,
                "bot_requests",
                ("bot_requests", "bot_like_requests", "bad_bot_requests"),
            )
        ),
        "human_requests": _num(
            _provenance_value(row, "human_requests", ("human_requests", "browser_requests"))
        ),
        "requests": requests,
        "bytes": bytes_value,
...
ORDER BY requests DESC, {value_field} ASC
LIMIT {int(top_n)}
""".strip()
...
def _hydrolix_usagemeter_discovery_sql(
    *,
    start: datetime,
    end: datetime,
    table_name: str,
    ...
    return f"""
SELECT
  catchall['project_deployment_id'] AS project_deployment_id,
  table_name,
  sum(toUInt64OrZero(toString(rows))) AS rows,
  sum(toUInt64OrZero(catchall['billing_bytes'])) AS billing_bytes,
  sum(toUInt64OrZero(toString(bytes))) AS raw_usage_bytes
...
WHERE timestamp >= toDateTime('{sql_ts(start)}', 'UTC')
  ...
  AND message = 'Reported usage.'
  AND table_name = {sql_literal(table_name)}
  AND nullIf(catchall['project_deployment_id'], '') IS NOT NULL
GROUP BY project_deployment_id, table_name
ORDER BY project_deployment_id, table_name
""".strip()


def _threat_hunt_summary_sql(
    *,
    database: str,
    table_name: str,
    start: datetime,
...
WITH
  ...
  toDateTime('{sql_ts(baseline_end)}', 'UTC') AS baseline_end
SELECT
  if(reqTimeSec >= current_start, 'current', 'baseline') AS period,
  toString(requestPath) AS request_path,
  toString(country) AS country,
  toString(trafficCohort) AS traffic_cohort,
  countMerge(`count()`) AS requests,
  countMergeIf(`count()`, trafficCohort IN ('Bot', 'AI')) AS bot_requests,
  countMergeIf(`count()`, trafficCohort NOT IN ('Bot', 'AI')) AS human_requests,
  sumMerge(`sum(bytes)`) AS response_body_bytes,
  sumMerge(`sum(totalBytes)`) AS akamai_billed_bytes,
  countMergeIf(`count()`, statusCode = 429) AS status_429,
  countMergeIf(`count()`, statusCode BETWEEN 500 AND 599) AS status_5xx
FROM {table}
WHERE (
  ...
  OR (
    reqTimeSec >= baseline_start
    AND reqTimeSec < baseline_end
  )
GROUP BY period, request_path, country, traffic_cohort
ORDER BY period, requests DESC, request_path ASC, country ASC, traffic_cohort ASC
LIMIT {SUMMARY_EXPORT_ROWS_PER_PERIOD} BY period
""".strip()


def _table_exists_sql(*, database: str, table_name: str) -> str:
    return f"""
SELECT
  count() AS table_count
FROM system.tables
WHERE database = {sql_literal(database)}
  AND name = {sql_literal(table_name)}
""".strip()


def _threat_hunt_legacy_summary_sql(
    *,
    database: str,
    table_name: str,
    start: datetime,
...
SELECT
  if(reqTimeSec >= current_start, 'current', 'baseline') AS period,
  toString(reqPath) AS request_path,
  toString(country) AS country,
  CAST(NULL, 'Nullable(String)') AS traffic_cohort,
  countMerge(`count()`) AS requests,
  CAST(NULL, 'Nullable(UInt64)') AS bot_requests,
  CAST(NULL, 'Nullable(UInt64)') AS human_requests,
  CAST(NULL, 'Nullable(UInt64)') AS response_body_bytes,
  sumMerge(`sum(totalBytes)`) AS akamai_billed_bytes,
  CAST(NULL, 'Nullable(UInt64)') AS status_429,
  countIfMerge(`countIf(and(greater(statusCode, 499), less(statusCode, 600)))`) AS status_5xx
FROM {table}
WHERE (
    reqTimeSec >= current_start
    AND reqTimeSec < current_end
...
GROUP BY period, request_path, country
ORDER BY period, requests DESC, request_path ASC, country ASC, traffic_cohort ASC
LIMIT {SUMMARY_EXPORT_ROWS_PER_PERIOD} BY period
""".strip()


def _threat_hunt_summary_candidate_count_sql(
    *,
    database: str,
    table_name: str,
    start: datetime,
    ...
    table = f"{database}.{table_name}"
    if source == "legacy_akamai_summary":
        path_column = "reqPath"
        country_column = "country"
        cohort_column = "CAST(NULL, 'Nullable(String)')"
    else:
        path_column = "requestPath"
        country_column = "country"
        cohort_column = "trafficCohort"
    return f"""
WITH
  toDateTime('{sql_ts(start)}', 'UTC') AS current_start,
  toDateTime('{sql_ts(end)}', 'UTC') AS current_end,
...
FROM (
  SELECT
    ...
    toString({country_column}) AS country,
    toString({cohort_column}) AS traffic_cohort
  FROM {table}
  WHERE (
      reqTimeSec >= current_start
      AND reqTimeSec < current_end
...
def export_impact_lane_totals(
    *,
    ...
    _write_rows_output(
        ...
        cluster=cluster,
        database=database,
        source_table=f"{database}.logs",
        query_sql=_impact_lane_windows_sql_text(database=database, windows=windows, user_agents=None),
        metadata={"scopes": list(windows), "chunk_seconds": chunk_seconds},
    )
    return rows
...
def export_impact_lane_scoped_hunt(
    *,
    ...
    _write_rows_output(
        ...
        database=database,
        source_table=f"{database}.logs",
        query_sql=_impact_lane_windows_sql_text(database=database, windows=windows, user_agents=user_agents),
        metadata={
            "scopes": list(windows),
            "chunk_seconds": chunk_seconds,
...
def _impact_lane_windows_sql_text(
    *,
    database: str,
...
ORDER BY requests DESC, user_agent ASC, client_ip ASC, request_path ASC, hour ASC{limit_clause}
""".strip()
...
FROM (
  SELECT
    ...
    row_number() OVER (
      PARTITION BY toString({user_agent_column})
      ORDER BY reqTimeSec, toString({client_ip_column}), toString({path_column}), statusCode
    ) AS ua_sample_rank
  FROM {database}.logs
  WHERE reqTimeSec >= toDateTime('{sql_ts(start)}', 'UTC')
    AND reqTimeSec < toDateTime('{sql_ts(end)}', 'UTC')
...
ORDER BY user_agent, reqTimeSec, client_ip, request_path, status_code
""".strip()
            ...
            for actor_type in ("client_ip", "user_agent"):
                ...
                _write_rows_output(
                    ...
                    metadata={
                        ...
                        "hash_buckets": actor_hash_buckets,
                        "topk_candidate_count": topk_candidate_count,
                    },
                )
    return output_dir
...
def _aggregate_hydrolix_usagemeter_export_rows(rows: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    cells: dict[tuple[str, str], dict[str, Any]] = {}
    for row in rows:
        project_deployment_id = str(_first(row, ("project_deployment_id",), "")).strip()
        table_name = str(_first(row, ("table_name",), "")).strip() or "logs"
        if not project_deployment_id:
            continue
        key = (project_deployment_id, table_name)
        cell = cells.setdefault(
            key,
            {
        ...
        window_start = str(_first(row, ("metadata_window_start", "window_start", "start"), "") or "")
        window_end = str(_first(row, ("metadata_window_end", "window_end", "end"), "") or "")
        if window_start and (not cell["metadata_window_start"] or window_start < cell["metadata_window_start"]):
            cell["metadata_window_start"] = window_start
        if window_end and (not cell["metadata_window_end"] or window_end > cell["metadata_window_end"]):
            cell["metadata_window_end"] = window_end
        cell["rows"] += _num(_first(row, ("rows", "usage_rows", "row_count")))
        cell["billing_bytes"] += _num(_first(row, ("billing_bytes", "sum_billing_bytes")))
        cell["raw_usage_bytes"] += _num(_first(row, ("raw_usage_bytes", "bytes", "sum_bytes")))
    merged: list[dict[str, Any]] = []
    for cell in cells.values():
        total_rows = _num(cell.get("rows"))
        billing_bytes = _num(cell.get("billing_bytes"))
        raw_usage_bytes = _num(cell.get("raw_usage_bytes"))
        cell["billing_bytes_per_row"] = billing_bytes / total_rows if total_rows > 0 else None
        cell["raw_usage_bytes_per_row"] = raw_usage_bytes / total_rows if total_rows > 0 and raw_usage_bytes > 0 else None
        for field in ("rows", "billing_bytes", "raw_usage_bytes"):
            value = _num(cell.get(field))
            cell[field] = int(value) if value.is_integer() else value
        merged.append(cell)
    return sorted(
        merged,
        key=lambda row: (str(row.get("project_deployment_id") or ""), str(row.get("table_name") or "")),
    )


def _positive_hydrolix_usagemeter_rows(rows: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        row
        for row in rows
        if _num(_first(row, ("rows", "usage_rows", "row_count"))) > 0
        and _num(_first(row, ("billing_bytes", "sum_billing_bytes"))) > 0
    ]


def select_hydrolix_usagemeter_project_deployment_id(
    rows: Iterable[dict[str, Any]],
    *,
    database: str,
    table_name: str = "logs",
) -> str:
    suffix = f"__{database}"
    matches = sorted(
        {
            str(_first(row, ("project_deployment_id",), "")).strip()
            for row in rows
            if str(_first(row, ("table_name",), "")).strip() == table_name
            and str(_first(row, ("project_deployment_id",), "")).strip().endswith(suffix)
            and _num(_first(row, ("rows", "usage_rows", "row_count"))) > 0
            and _num(_first(row, ("billing_bytes", "sum_billing_bytes"))) > 0
        }
    )
    if not matches:
        raise SystemExit(
            f"No hydro.logs usagemeter deployment matched table_name={table_name!r} "
            f"and project_deployment_id suffix {suffix!r}."
        )
    if len(matches) > 1:
        raise SystemExit(
            "Multiple hydro.logs usagemeter deployments matched "
            f"table_name={table_name!r} and project_deployment_id suffix {suffix!r}: "
            + ", ".join(matches)
        )
    return matches[0]


def _hydrolix_usagemeter_windows(
    *,
    start: datetime,
    end: datetime,
    baseline_start: datetime | None,
    baseline_end: datetime | None,
    chunk_seconds: int = HYDROLIX_USAGEMETER_MAX_SECONDS,
) -> list[tuple[datetime, datetime]]:
    if chunk_seconds <= 0:
        raise SystemExit("hydrolix usagemeter chunk_seconds must be positive")
...
def export_hydrolix_usagemeter_ingest_estimate(
    *,
    ...
    cluster: str,
    project_deployment_id: str | None = None,
    ...
    baseline_start_dt = parse_time(baseline_start, "baseline-start") if baseline_start else None
    baseline_end_dt = parse_time(baseline_end, "baseline-end") if baseline_end else None
    windows = _hydrolix_usagemeter_windows(
        start=start_dt,
        end=end_dt,
    ...
    output_path = Path(output).expanduser().resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    discovery_sqls: list[str] = []
    export_sqls: list[str] = []
    with tempfile.TemporaryDirectory(prefix="threat-hunt-usagemeter-") as tmpdir:
        tmp = Path(tmpdir)
        if not project_deployment_id:
            if not database:
                raise SystemExit(
                    "database is required to discover a hydro.logs usagemeter project_deployment_id."
                )
            discovery_rows: list[dict[str, Any]] = []
            for index, (chunk_start, chunk_end) in enumerate(windows, start=1):
                chunk_output = tmp / f"usagemeter-discovery-{index}.json"
                discovery_sql = _hydrolix_usagemeter_discovery_sql(
                    start=chunk_start,
                    end=chunk_end,
                    table_name=table_name,
                )
                discovery_sqls.append(discovery_sql)
                _run_mux_export(
                    cluster,
                    discovery_sql,
                    chunk_output,
                )
                discovery_rows.extend(_read_json_rows(chunk_output))
            project_deployment_id = select_hydrolix_usagemeter_project_deployment_id(
                _aggregate_hydrolix_usagemeter_export_rows(discovery_rows),
                database=database,
                table_name=table_name,
            )
        export_rows: list[dict[str, Any]] = []
        for index, (chunk_start, chunk_end) in enumerate(windows, start=1):
            chunk_output = tmp / f"usagemeter-export-{index}.json"
            export_sql = _hydrolix_usagemeter_sql(
                start=chunk_start,
                end=chunk_end,
                project_deployment_id=project_deployment_id,
                table_name=table_name,
            )
            export_sqls.append(export_sql)
            _run_mux_export(
                cluster,
                export_sql,
                chunk_output,
            )
            export_rows.extend(_read_json_rows(chunk_output))
    merged = _aggregate_hydrolix_usagemeter_export_rows(export_rows)
    if not _positive_hydrolix_usagemeter_rows(merged):
        raise SystemExit(
            "hydro.logs usagemeter export did not produce positive rows and billing_bytes."
        )
    write_threat_hunt_dataset(
        output_path,
        ...
        query_sql="\n\n-- discovery query\n".join(discovery_sqls + export_sqls),
        rows=merged,
        field_provenance=_field_provenance(
            _columns_for_rows(merged),
            default=PROVENANCE_ESTIMATED,
            overrides={
                "source": PROVENANCE_OBSERVED,
                "project_deployment_id": PROVENANCE_OBSERVED,
                "table_name": PROVENANCE_OBSERVED,
                "metadata_window_start": PROVENANCE_OBSERVED,
                "metadata_window_end": PROVENANCE_OBSERVED,
                "rows": PROVENANCE_OBSERVED,
                "billing_bytes": PROVENANCE_OBSERVED,
                "raw_usage_bytes": PROVENANCE_OBSERVED,
            },
        ),
        ...
        metadata={
            "formula_provenance": {
                "billing_bytes_per_row": {
                    "formula": "billing_bytes / rows",
                    "source_fields": ["billing_bytes", "rows"],
                    "source_table": "hydro.logs",
                },
                "raw_usage_bytes_per_row": {
                    "formula": "raw_usage_bytes / rows",
                    "source_fields": ["raw_usage_bytes", "rows"],
                    "source_table": "hydro.logs",
                },
            },
            "query_chunks": {
                "discovery_count": len(discovery_sqls),
                "export_count": len(export_sqls),
                "windows": [
                    {
...
def _run_mux_export(cluster: str, sql: str, output: Path) -> None:
    ...
    stage_id = _PROVENANCE_STAGE_STACK[-1] if _PROVENANCE_STAGE_STACK else "mux_export"
    result = None
    try:
    ...
    if output.exists():
        try:
            output_rows = len(_read_json_rows(output))
        except Exception:
            output_rows = None
    _record_provenance_event(
        {
            ...
            "stdout_summary": (result.stdout or "").strip()[:4000],
            "stderr_summary": (result.stderr or "").strip()[:4000],
        }
    )
    if result.returncode != 0:
...
def _mux_table_exists(*, cluster: str, database: str, table_name: str) -> bool:
    with tempfile.NamedTemporaryFile("w", encoding="utf-8", suffix=".json", delete=False) as handle:
        output = Path(handle.name)
    try:
        _run_mux_export(
            cluster,
            _table_exists_sql(database=database, table_name=table_name),
            output,
        )
        rows = _read_json_rows(output)
    finally:
        output.unlink(missing_ok=True)
    return any(_num(_first(row, ("table_count", "count()"))) > 0 for row in rows)


def export_threat_hunt_summary(
    *,
    output: str,
    start: str,
    end: str,
    ...
    planned_table: str | None = None,
) -> Path:
    if granularity not in {"minute", "hour", "day"}:
        raise SystemExit("threat-hunt summary granularity must be minute, hour, or day")
    output_path = Path(output).expanduser().resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    source = "bot_insights_summary"
    table_name = f"bi_summary_{granularity}"
    if planned_table:
        planned_prefix = f"{database}."
        if not planned_table.startswith(planned_prefix):
            raise SystemExit(
                f"Planned threat-hunt summary table {planned_table!r} is not in database {database!r}."
            )
        table_name = planned_table[len(planned_prefix):]
        if table_name == "summary_hour":
            source = "legacy_akamai_summary"
        elif table_name != f"bi_summary_{granularity}":
            raise SystemExit(
                f"Planned threat-hunt summary table {planned_table!r} does not match granularity {granularity!r}."
            )
        if not _mux_table_exists(cluster=cluster, database=database, table_name=table_name):
            raise SystemExit(f"Could not find planned threat-hunt summary table {planned_table}.")
    elif not _mux_table_exists(cluster=cluster, database=database, table_name=table_name):
        if not allow_legacy_fallback:
            raise SystemExit(
                f"Could not find planned threat-hunt summary table {database}.{table_name}; "
                "full-required harvest plans do not allow implicit legacy fallback."
            )
        legacy_tables = {"minute": "summary_hour", "hour": "summary_hour"}
        legacy_table = legacy_tables.get(granularity)
        if legacy_table and _mux_table_exists(
            cluster=cluster, database=database, table_name=legacy_table
        ):
            source = "legacy_akamai_summary"
            table_name = legacy_table
        else:
            raise SystemExit(
                f"Could not find {database}.bi_summary_{granularity}"
                + (f" or {database}.{legacy_table}" if legacy_table else "")
                + " for threat-hunt summary export."
            )
    summary_sql = (
        _threat_hunt_legacy_summary_sql
        if source == "legacy_akamai_summary"
        else _threat_hunt_summary_sql
    )
    _record_provenance_event(
        {
            "event_type": "decision",
            ...
            "rationale": "Threat-hunt summary export uses Bot Insights summary tables when present and falls back to legacy Akamai summary tables for deployments without bi_summary_*.",
            "source": source,
            "table": f"{database}.{table_name}",
            "granularity": granularity,
        }
    )
    query_sql = summary_sql(
        database=database,
        table_name=table_name,
    ...
    candidate_count_sql = _threat_hunt_summary_candidate_count_sql(
        database=database,
        table_name=table_name,
        start=parse_time(start, "start"),
        end=parse_time(end, "end"),
    ...
    with tempfile.NamedTemporaryFile("w", encoding="utf-8", suffix=".json", delete=False) as handle:
        candidate_count_path = Path(handle.name)
    try:
        _run_mux_export(cluster, candidate_count_sql, candidate_count_path)
        candidate_count_rows = _read_json_rows(candidate_count_path)
    finally:
        candidate_count_path.unlink(missing_ok=True)
    candidate_rows_by_period = {
        str(_first(row, ("period",), "")).lower(): int(_num(_first(row, ("candidate_rows", "count()"), 0)))
        for row in candidate_count_rows
    }
    candidate_row_count = sum(candidate_rows_by_period.values())
    _run_mux_export(
        cluster,
        query_sql,
        output_path,
    )
    rows = _read_json_rows(output_path)
    if not rows or not any(_num(_first(row, ("requests", "request_count", "count", "hits"))) > 0 for row in rows):
        raise SystemExit("threat-hunt summary export produced no usable rows.")
    periods = {str(_first(row, ("period",), "")).lower() for row in rows}
    if not {"current", "baseline"} <= periods:
        raise SystemExit("threat-hunt summary export must include both current and baseline rows.")
    columns = _columns_for_rows(rows)
    field_overrides = {}
    if source == "legacy_akamai_summary":
        field_overrides = {
            "traffic_cohort": PROVENANCE_SYNTHETIC_UNAVAILABLE,
            "bot_requests": PROVENANCE_SYNTHETIC_UNAVAILABLE,
            "human_requests": PROVENANCE_SYNTHETIC_UNAVAILABLE,
            "response_body_bytes": PROVENANCE_SYNTHETIC_UNAVAILABLE,
            "status_429": PROVENANCE_SYNTHETIC_UNAVAILABLE,
        }
    write_threat_hunt_dataset(
        output_path,
        stage_id="summary_export",
        ...
        query_sql=query_sql,
        rows=rows,
        field_provenance=_field_provenance(columns, overrides=field_overrides),
        total_row_count=candidate_row_count,
        truncated=len(rows) < candidate_row_count,
        metadata={
            "population_complete": False,
            "evidence_role": "selected_summary_evidence",
            "selection": {
                "mode": "top_n_by_period",
                "limit_per_period": SUMMARY_EXPORT_ROWS_PER_PERIOD,
                "population_complete": False,
                "evidence_role": "selected_summary_evidence",
                "order_by": [
                    "period",
                ...
                "candidate_rows_by_period": candidate_rows_by_period,
                "candidate_row_count": candidate_row_count,
                "selected_row_count": len(rows),
                "selection_sql_sha256": hashlib.sha256(query_sql.encode("utf-8")).hexdigest(),
                "candidate_count_sql_sha256": hashlib.sha256(candidate_count_sql.encode("utf-8")).hexdigest(),
            }
        },
    )
    return output_path
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="threat-hunt-cooccurrence-") as tmpdir:
        ...
        for index, (chunk_start, chunk_end) in enumerate(
            ...
            query_sql = _raw_cooccurrence_sql(
                database=database,
                start=chunk_start,
                end=chunk_end,
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="threat-hunt-scraper-drilldown-") as tmpdir:
        ...
        for index, (chunk_start, chunk_end) in enumerate(
            ...
            chunk_output = tmp / f"chunk-{index}.json"
            query_sql = _raw_scraper_drilldown_sql(
                database=database,
                start=chunk_start,
                end=chunk_end,
                client_ips=client_ips,
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="threat-hunt-scraper-hourly-") as tmpdir:
        ...
        for index, (chunk_start, chunk_end) in enumerate(chunks, start=1):
            ...
            query_sql = _raw_scraper_hourly_sql(
                database=database,
                start=chunk_start,
                end=chunk_end,
                user_agents=user_agents,
...
def export_fanout_enrichment(
    *,
    ...
    query_sql = _fanout_replay_query_sql(
        database=database,
        start_dt=start_dt,
        end_dt=end_dt,
        user_agents=user_agents,
    ...
    _write_rows_output(
        ...
        database=database,
        source_table=f"{database}.logs"
        if selected_strategy == "logs_probe"
        else f"{database}.summary_hour"
        if selected_strategy == "summary_hour"
        else None,
        query_sql=query_sql,
        metadata={
            "selected_user_agents": user_agents,
            ...
            "cooccurrence_input": cooccurrence_in,
            "scraper_hourly_input": scraper_hourly_in,
        },
    )
    if run_summary is not None:
        run_summary.clear()
...
def export_background_ua_sample(
    *,
    ...
    if chunk_seconds <= 0:
        raise SystemExit("background UA sample chunk_seconds must be positive")
    output_path = Path(output).expanduser().resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    start_dt = parse_time(start, "start")
    end_dt = parse_time(end, "end")
    ...
    chunk_row_counts: list[int] = []
    query_sqls: list[str] = []
    with tempfile.TemporaryDirectory(prefix="threat-hunt-background-ua-") as tmpdir:
        tmp = Path(tmpdir)
        for index, (chunk_start, chunk_end) in enumerate(chunks, start=1):
            chunk_output = tmp / f"chunk-{index}.json"
            query_sql = _raw_background_ua_sample_sql(
                database=database,
                start=chunk_start,
                end=chunk_end,
    ...
    for row in chunk_rows:
        user_agent = str(_first(row, ("user_agent", "UA", "ua"), "")).strip()
        if not user_agent:
            continue
        merged = by_ua.setdefault(
            user_agent,
            {
                "user_agent": user_agent,
                "requests": 0,
                "unique_client_ips": 0,
                "targeted_endpoint_requests": 0,
                "status_429": 0,
                "status_5xx": 0,
                "any_path": _first(row, ("any_path", "request_path", "path"), ""),
                "source": "chunked_logs",
            },
        )
        merged["requests"] = _num(merged.get("requests")) + _num(row.get("requests"))
        merged["unique_client_ips"] = max(
            _num(merged.get("unique_client_ips")),
            _num(row.get("unique_client_ips")),
        )
        merged["targeted_endpoint_requests"] = _num(
            merged.get("targeted_endpoint_requests")
        ) + _num(row.get("targeted_endpoint_requests"))
        merged["status_429"] = _num(merged.get("status_429")) + _num(row.get("status_429"))
        merged["status_5xx"] = _num(merged.get("status_5xx")) + _num(row.get("status_5xx"))
    rows = [
        row
        for row in by_ua.values()
        if _num(row.get("requests")) >= 100 and _num(row.get("requests")) <= 10_000
    ]
    rows.sort(key=lambda row: str(row.get("user_agent")))
...
def export_baseline_ua_timeseries(
    *,
    ...
    if chunk_seconds <= 0:
        raise SystemExit("baseline UA timeseries chunk_seconds must be positive")
    output_path = Path(output).expanduser().resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    start_dt = parse_time(baseline_start, "baseline-start")
    end_dt = parse_time(baseline_end, "baseline-end")
    chunks = split_raw_cooccurrence_window(start_dt, end_dt, max_seconds=chunk_seconds)
    chunk_rows: list[dict[str, Any]] = []
    chunk_row_counts: list[int] = []
    query_sqls: list[str] = []
    with tempfile.TemporaryDirectory(prefix="threat-hunt-baseline-ua-") as tmpdir:
        tmp = Path(tmpdir)
        for index, (chunk_start, chunk_end) in enumerate(chunks, start=1):
            chunk_output = tmp / f"chunk-{index}.json"
            query_sql = _raw_baseline_ua_timeseries_sql(
                database=database,
                baseline_start=chunk_start,
                baseline_end=chunk_end,
                user_agents=user_agents,
                granularity=granularity,
            )
            query_sqls.append(query_sql)
    ...
    for row in chunk_rows:
        user_agent = str(_first(row, ("user_agent", "UA", "ua"), "")).strip()
        bucket = str(_first(row, ("bucket",), "")).strip()
        if not user_agent or not bucket:
            continue
        merged = by_key.setdefault(
            (user_agent, bucket),
            {"user_agent": user_agent, "bucket": bucket, "requests": 0},
        )
        merged["requests"] = _num(merged.get("requests")) + _num(row.get("requests"))
    rows = sorted(by_key.values(), key=lambda row: (str(row.get("user_agent")), str(row.get("bucket"))))
    _write_rows_output(
        output_path,
        rows,
        replay_grade=replay_grade,
...
def _read_optional_rows(path_value: str | None) -> list[dict[str, Any]]:
    ...
    if path.suffix in {".json", ".jsonl"}:
        return _attach_dataset_manifest(_read_json_rows(path), _read_dataset_manifest(path))
    if path.suffix == ".csv":
        return _attach_dataset_manifest(_read_csv_rows(path), _read_dataset_manifest(path))
    if path.suffix == ".parquet":
        return _attach_dataset_manifest(_read_parquet_rows(path), _read_dataset_manifest(path))
...
def _hydrolix_usagemeter_estimate_from_rows(
    ...
    *,
    ...
    return {
        ...
        "formula": {
            "name": "requests * billing_bytes_per_row",
            "billing_bytes_per_row_source": "hydro.logs usage-meter billing_bytes / hydro.logs usage-meter rows",
            "source_rows": total_rows,
            "source_billing_bytes": billing_bytes,
            "source_raw_usage_bytes": raw_usage_bytes,
        },
...
def merge_impact_lanes_into_artifact(
    ...
    *,
    ...
    if current_total_lane and baseline_total_lane:
        impact["totals_source"] = "raw_log_impact_lanes"
        impact["totals_population_complete"] = True
        impact["totals_evidence_role"] = "population_complete_raw_log_totals"
        impact["totals_source_detail"] = {
        ...
        if isinstance(artifact_metadata, dict):
            artifact_metadata["population_totals"] = {
...
def _validate_summary_period_coverage(rows: list[dict[str, Any]]) -> None:
    periods = {str(row.get("period") or "").lower() for row in rows if _num(row.get("requests")) > 0}
    missing = sorted({"current", "baseline"} - periods)
    if missing:
        raise SystemExit(
            "threat-hunt summary input must include non-empty rows for both current and baseline; "
            "missing " + ", ".join(missing)
        )


def _validate_replay_grade_optional(path_value: str | None, *, role: str) -> None:
    if path_value:
        validate_replay_grade_dataset(Path(path_value).expanduser().resolve(), role=role)


def _validate_replay_grade_actor_dir(raw_actor_dir: str | None) -> None:
    if not raw_actor_dir:
        return
    actor_paths = sorted(Path(raw_actor_dir).expanduser().glob("*.json"))
    if not actor_paths:
        raise SystemExit(f"full-required raw_actor input directory has no JSON files: {raw_actor_dir}")
    for path in actor_paths:
        validate_replay_grade_dataset(path.resolve(), role="raw_actor")
...
def build_threat_hunt_artifact(
    *,
    ...
    require_replay_grade: bool = False,
) -> dict[str, Any]:
    ...
    if require_replay_grade:
        validate_replay_grade_glob(summary_parquet_glob, role="summary")
        _validate_replay_grade_actor_dir(raw_actor_dir)
        _validate_replay_grade_optional(cooccurrence_in, role="cooccurrence")
        _validate_replay_grade_optional(scraper_drilldown_in, role="scraper_drilldown")
        _validate_replay_grade_optional(scraper_hourly_in, role="scraper_hourly")
        _validate_replay_grade_optional(fanout_in, role="fanout")
        _validate_replay_grade_optional(background_ua_sample_in, role="background_ua_sample")
        _validate_replay_grade_optional(baseline_ua_timeseries_in, role="baseline_ua_timeseries")
        if hydrolix_log_ingest_usagemeter_in:
            validate_replay_grade_dataset(
                Path(hydrolix_log_ingest_usagemeter_in).expanduser().resolve(),
                role="hydrolix_usagemeter",
            )
    rows = [_normalize_summary_row(row) for row in read_rows_from_glob(summary_parquet_glob)]
    if require_replay_grade:
        _validate_summary_period_coverage(rows)
    ...
    impact_assessment.setdefault("totals_source", "selected_summary_evidence")
    impact_assessment.setdefault("totals_population_complete", False)
    impact_assessment.setdefault("totals_evidence_role", "selected_summary_evidence")

    summary_dataset_metadata = [
        row.get("__dataset_metadata")
        for row in rows
        if isinstance(row.get("__dataset_metadata"), dict)
    ]
    summary_selection_metadata = [
        metadata.get("metadata")
        for metadata in summary_dataset_metadata
        if isinstance(metadata.get("metadata"), dict)
    ]
    summary_evidence_metadata = {
        "evidence_role": "selected_summary_evidence",
        "population_complete": False,
        "usage": "ranking_and_selected_evidence",
        "dataset_count": len(summary_dataset_metadata),
        "datasets": summary_dataset_metadata,
    }
    if summary_selection_metadata:
        summary_evidence_metadata["selection"] = summary_selection_metadata[0].get("selection")
    ...
    return {
        ...
        "artifact_metadata": {
            "summary_evidence": summary_evidence_metadata,
            "population_totals": {
                "source": impact_assessment["totals_source"],
                "population_complete": impact_assessment["totals_population_complete"],
                "note": "Summary rows are bounded selected evidence; population-complete request and byte totals require raw-log impact lanes.",
            },
        },
