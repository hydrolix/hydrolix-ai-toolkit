...
def _temporal_progression_view(scope_art: dict) -> dict:
    ...
    bullets: list[str] = []
    if current:
        start = _parse_ts(volume.get("start") or (scope_art.get("scope") or {}).get("start"))
        end = _parse_ts(volume.get("end") or (scope_art.get("scope") or {}).get("end"))
        step = _granularity_delta(volume.get("granularity") or (scope_art.get("scope") or {}).get("granularity"))
